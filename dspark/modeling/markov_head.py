...
class VanillaMarkov(nn.Module):
    def __init__(self, *, vocab_size: int, markov_rank: int):
        ...
        self.markov_rank = int(markov_rank) # 马尔可夫维度 低秩
        self.markov_head_type = "vanilla"
        assert self.markov_rank > 0, (
            f"VanillaMarkov requires markov_rank > 0, got {self.markov_rank}."
        )
        self.markov_w1 = nn.Embedding(self.vocab_size, self.markov_rank) # token -> 低秩向量
        self.markov_w2 = nn.Linear(self.markov_rank, self.vocab_size, bias=False) # 低秩向量 -> 整个词表
    ...
    def compute_step_bias(
        ...
        del hidden_states
        # 低秩矩阵乘法 两次 其实就是先embedding proj 为什么分开也是显而易见了，不分开维度就是v * v了
        return self.project_bias(self.get_prev_embeddings(token_ids))
    # 拿到的映射 对应加到相应的logits里面
    ...
    def apply_block_logits( # 整块并行加偏置
        self,
    ...
    def sample_block_tokens(
        ...
        *,
        ...
        if proposal_len == 0:
            ...
            return empty_tokens, base_logits # 空块保护

        sampled_tokens = [] # 攒每步采样出的token
        corrected_logits = [] # 攒美步加完偏置的logits
        prev_token_ids = first_prev_token_ids.long() # 第一个已知的token
        for step_idx in range(proposal_len): # 串行 逐位置开始采样
            step_hidden = None if hidden_states is None else hidden_states[:, step_idx, ...]
            step_logits = self.apply_step_logits( # 给这步logits加上markov偏置
                base_logits[:, step_idx, :],
                token_ids=prev_token_ids,
                hidden_states=step_hidden,
            )
            corrected_logits.append(step_logits.unsqueeze(1))
            next_token_ids = sample_tokens( # 采样出这个位置的token
