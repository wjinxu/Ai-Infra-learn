...
class Qwen3DSparkModel(Qwen3PreTrainedModel):
    ...
    def sample_draft_tokens(
        ...
        *,
        ...
        if self.markov_head is None: # 如果没有 markov头就采取简单采样
            return sample_tokens(base_logits, temperature), base_logits
        return self.markov_head.sample_block_tokens( # 否则使用 markov_head
            base_logits,
    ...
    def forward(
        ...
        output_hidden = self._forward_backbone( # 主干得到hidden
        ...
        output_hidden_4d = output_hidden.reshape(bsz, num_blocks, self.block_size, -1) # 重排成块结构
        ...
        if self.markov_head is not None: # markov 头
        ...
        if self.confidence_head is not None:
            if self.confidence_head_with_markov: # 如果有markov的话 把markov embedding也加进去
                prev_embeddings = self.markov_head.get_prev_embeddings(prev_token_ids).to(
                ...
                confidence_pred = self.confidence_head(output_hidden_4d).float() # 如果没有的话就只给output
