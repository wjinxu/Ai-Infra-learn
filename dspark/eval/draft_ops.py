...
def forward_dspark_draft_block(
    model: DSparkModel,
    *,
    draft_input_ids: torch.Tensor, # 当前块已知的参数 也就是noisy token [已知token，mask， mask， ...]
    position_ids: torch.Tensor, # 位置编码
    past_key_values_draft: DynamicCache, # kv
    target_hidden_states: torch.Tensor, # 隐藏层维度
    start: int,
    block_size: int,
) -> torch.Tensor:
    ...
    block_hidden = model._forward_backbone( # 调用backbone 也就是dflash
